...
QuizAi는 유튜브 영상을 통해 학습한 내용을 퀴즈로 풀며 복습하면 학습 효과가 배가될 것이라는 생각에서 시작한 프로젝트입니다.

개발 초기에는 단순히 URL 입력만으로 모든 과정이 처리될 것이라 예상했으나, 실제 구현 과정에서 기술적 난관에 부딪혔습니다. 
API가 직접 영상을 처리하지 못해 `yt-dlp`를 활용한 전처리 파이프라인을 구축해야 했고, 
긴 처리 시간으로 인한 타임아웃 문제를 해결하기 위해 비동기 처리 구조를 도입해야 했습니다.

이처럼 예상치 못한 제약 사항들을 마주했지만, 검색과 AI를 활용해 문제를 하나씩 해결해 나가는 과정 자체가 큰 성장이었습니다. 
단순히 기능을 구현하는 것을 넘어, 안정적인 서비스를 만들기 위해 아키텍처를 고민했던 이 경험은 앞으로의 개발 인생에 큰 자산이 될 것입니다.